...
function paypalForm($order_res) {
  ...
  //Usd Rates
  $usd = file_get_contents('http://api.fixer.io/latest?base=INR');
  $usd = json_decode($usd);
  $usdRate = $usd->rates->USD;
  if($order_res['shipping_country']!='India'){
    $amount = $order_res['total_amount'];
    $amount = $amount*$usdRate+$order_res['shipping_amount'];
  }
  else{
    $amount = $order_res['total_amount']+$order_res['shipping_amount'];
  }
  ...
  <body style="margin:0px;">    
    ...
    <form name="form1" action="<?php echo $paypalWeb; ?>" method="post"> 
      ...
      <input type="hidden" name="business" value="mayank.jethwani310891@gmail.com"> 
