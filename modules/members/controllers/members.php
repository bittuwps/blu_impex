...
class Members extends Private_Controller {
  ...
  public function remove_wishlist() {
    ...
    if ($wish_id > 0) {
      $record = count_record('wl_wishlists', "id ='" . $wish_id . "'");
      if ($record > 0) {
        $this->db->query("DELETE FROM wl_wishlists WHERE id = '" . $wish_id . "'");
  ...
  public function cancel_order() {
    ...
    if ($orderid > 0) {
      $this->db->query("UPDATE wl_order SET order_status = 'Canceled' WHERE order_id = '" . $orderid . "'");
