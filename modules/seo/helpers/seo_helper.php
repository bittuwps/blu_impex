            ...
            if (is_object($res)) {
                ...
                if ($ci->router->fetch_class() == 'category' || $ci->router->fetch_class() == 'products') {
                    if ($ci->router->fetch_class() == 'category') {
                        ...
                        return array(
                            "meta_title" => "$nm Manufacturers in Delhi, Wholesale $nm Suppliers, Exporters India",
                            "meta_keyword" => "$nm Manufacturers in Delhi, Wholesale $nm in Delhi, $nm Exporters India, $nm Suppliers in Delhi",
                            "meta_description" => "$nm Manufacturers in Delhi - Shri Ank Enterprise Pvt. Ltd Most Trusted $nm wholesale suppliers, Best Quality $nm exporters India. Leading The Industry since 2006. Best Price.",
                            "entity_type" => $res->entity_type,
                            "entity_id" => $res->entity_id,
                            "page_url" => $res->page_url,
                        );
                        ...
                        return array(
                            "meta_title" => "$nm Manufacturers, Suppliers, Exporters in $subdomain_location",
                            "meta_keyword" => "$nm, $nm manufacturers, $nm suppliers, $nm suppliers in $subdomain_location",
                            "meta_description" => "Get the best quality $nm at best price from  Shri Ank Enterprise, leading $nm Manufacturer Wholesaler, Suppliers, Exporters of India. Get $nm detail here.",
                            "entity_type" => $res->entity_type,
                            "entity_id" => $res->entity_id,
                            "page_url" => $res->page_url,
                        );
                    ...
                    return array(
                        "meta_title" => "$nm Manufacturers, Suppliers, Exporters in $subdomain_location",
                        "meta_keyword" => "$nm Manufacturers, Suppliers, Exporters in $subdomain_location",
                        "meta_description" => "$nm Manufacturers, Suppliers, Exporters in $subdomain_location",
                        "entity_type" => $res->entity_type,
                        "entity_id" => $res->entity_id,
                        "page_url" => $res->page_url,
                    );
