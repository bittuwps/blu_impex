            ...
            if (is_object($res)) {
                if ($ci->router->fetch_class() == 'category' || $ci->router->fetch_class() == 'products') {
                    if ($ci->router->fetch_class() == 'category') {
                        ...
                        return array(
                            "meta_title" => 'Buy ' . $nm . ' Online, Single Pc from ',
                            "meta_keyword" => 'Buy ' . $nm . ' Online, Single Pc from ',
                            "meta_description" => 'Buy ' . $nm . ' Online, Single Pc from ',
                            "entity_type" => $res->entity_type,
                            "entity_id" => $res->entity_id,
                            "page_url" => $res->page_url,
                        );
                    ...
                    //echo_sql();
                    return array(
                        "meta_title" => "$nm Services, $nm Services Delhi, $nm Services India",
                        "meta_keyword" => "We leading $nm Delhi India providing $nm Services, $nm Services Delhi, $nm Services India, $nm Consultant India",
                        "meta_description" => "We leading $nm Delhi India providing $nm Services, $nm Services Delhi, $nm Services India, $nm Consultant India",
                        "entity_type" => $res->entity_type,
                        "entity_id" => $res->entity_id,
                        "page_url" => $res->page_url,
                    );
