...
class Products extends Public_Controller
{
    ...
    public function index()
    {
        ...
        $data['catleftRes'] = $this->db->query("SELECT category_id, category_name, friendly_url FROM wl_categories WHERE status='1' AND parent_id = '$category_id' GROUP BY category_id")->result_array();
    ...
    public function get_product_price()
    {
        ...
        if ($sid > 0) {
            $res = $this->db->select('quantity,quantity,product_price,product_discounted_price')->get_where('wl_product_attributes', array('size_id' => $sid, 'product_id' => $pid))->row();
            //echo_sql();
            if (is_object($res)) {
