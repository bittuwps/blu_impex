...
class Staticpages extends Admin_Controller {
  ...
  public function edit() {
    ...
    $data['ckeditor1'] = set_ck_config(array('textarea_id' => 'page_desc_p'));
    ...
    if (is_array($res)) {
      ...
      //$this->form_validation->set_rules('page_description_p', 'Description in Portuguese', 'required|max_length[8500]');

      if ($this->form_validation->run() == TRUE) {
        $posted_data = array(
            'page_description' => $this->input->post('page_description', TRUE),
            //'page_description_p' => $this->input->post('page_description_p', TRUE),
