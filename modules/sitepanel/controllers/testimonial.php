...
class Testimonial extends Admin_Controller {
  ...
  public function post() {
    ...
    $this->form_validation->set_rules('image1', 'Testimonial Image', "required|file_allowed_type[image]");
    $this->form_validation->set_rules('testimonial_description', 'Success Story', 'trim|required|xss_clean|max_length[8500]');
    $this->form_validation->set_rules('testimonial_description_p', 'Success Story in Portuguese', 'trim|required|xss_clean|max_length[8500]');
    ...
    if ($this->form_validation->run() == TRUE) {
      ...
      if (!empty($_FILES) && $_FILES['image1']['name'] != '') {
        ...
        $uploaded_data = $this->upload->my_upload('image1', 'testimonial');
      ...
      $posted_data = array(
          ...
          'testimonial_description_p' => $this->input->post('testimonial_description_p'),
          'status' => '1',
  ...
  public function edit() {
    ...
    if (is_array($res) && !empty($res)) {
      ...
      if ($this->form_validation->run() == TRUE) {
        ...
        $unlink_image = array('source_dir' => "testimonial", 'source_file' => $uploaded_file);
