  ...
  function display_price($price) {
    ...
    if ($price >= 0) {
      if ($CI->session->userdata('currency_id')) {
        $res = $CI->db->query("SELECT * FROM wl_currencies WHERE currency_id='" . $CI->session->userdata('currency_id') . "' AND status='1' ")->row();
      } else {
        $res = $CI->db->query("SELECT * FROM wl_currencies WHERE is_default='Y' AND status='1' ")->row();
      }
      if (is_object($res)) {
        ...
        if ($currency_id != "" && $value != "") {
          ...
          $final_price = ( $symbol_left != '') ? $symbol_left . ' ' . number_format($new_price, 2) : number_format($new_price, 2);
        } else {
          $final_price = ($res->symbol_left != '') ? $symbol_left . ' ' . number_format($price, 2) : number_format($price, 2);
        }
      }
      return $final_price;
    }
